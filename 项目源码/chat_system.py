...
def get_relevant_chain(input_text):
...
# 定义一个处理数据库查询的函数
def handle_database_query(input_text):
...
def generate_query(input_text):
    if "库存" in input_text:
        ...
        return "SELECT Tea_Orders.id, Users.username, Tea_Orders.total_amount FROM Tea_Orders JOIN Users ON TeaOrders.user_id = Users.id"
