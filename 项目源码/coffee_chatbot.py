...
def handle_coffee_chat(input_text):
...
# 定义一个处理数据库查询的函数
def handle_coffee_database_query(input_text):
...
def generate_query(input_text):
    if "库存" in input_text:
        return "SELECT name, stock FROM Coffee_Goods"
    elif "订单" in input_text:
        return "SELECT Coffee_Orders.id, Users.username, Coffee_Orders.total_amount FROM Coffee_Orders JOIN Users ON Coffee_Orders.user_id = Users.id"
    elif "用户" in input_text:
        return "SELECT * FROM Users"
    elif "商品" in input_text:
        return "SELECT * FROM Coffee_Goods"
    else:
