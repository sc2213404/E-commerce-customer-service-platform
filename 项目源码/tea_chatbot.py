...
def handle_tea_chat(input_text):
...
# 定义一个处理数据库查询的函数
def handle_tea_database_query(input_text):
...
def generate_query(input_text):
    if "库存" in input_text:
        return "SELECT name, stock FROM Tea_Goods"
    elif "订单" in input_text:
        return "SELECT Tea_Orders.id, Users.username, Tea_Orders.total_amount FROM Tea_Orders JOIN Users ON Tea_Orders.user_id = Users.id"
    elif "用户" in input_text:
        return "SELECT * FROM Users"
    elif "商品" in input_text:
        return "SELECT * FROM Tea_Goods"
    else:
